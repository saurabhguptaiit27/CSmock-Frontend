...
const ExpertBookings = ({
  ...
  const handleConcludeClick = async (bookingId) => {
    const response = await fetch(
      `https://csmock-backend.onrender.com/api/v1/users-experts/concludebooking?string=${encodeURIComponent(
        bookingId
      )}`,
      {
        method: "POST",
    ...
    <section className="h-auto w-auto min-h-screen bg-gray-950/90 py-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:px-16">
        {bookingDetails.map((bookingData, index) => (
          <div
            key={index}
            className=" border-2  border-gray-400/80 rounded-2xl px-5 py-3 mb-5 hover:bg-gray-950/50
            hover:shadow-xl hover:shadow-gray-100/40 hover:border-transparent transition-colors duration-300 transform"
          >
